...
int main()
{
	...
	// 인증서 “cert” 검증을 위하여 “vrfyCertCtx”를 초기화.
	// 		CA 인증서는 store에 저장되어 있음.
	// 각 인증서 검증시마다 호출하여야 함.
	X509_STORE_CTX_init(vfyCrtCtx, store, cert, NULL);
	
	// 성공시 1, 실패시 0을 리턴
	ret = X509_verify_cert(vfyCrtCtx);
	BIO_printf(outBio, "Verification return code: %d\n", ret);
	if (ret == 0 || ret == 1)
		BIO_printf(outBio, "Verification result text: %s\n",
			X509_verify_cert_error_string(X509_STORE_CTX_get_error (vfyCrtCtx)));
	
	// 실패시
	if (ret == 0) {
		// 실패한 인증서를 가져와서 출력
		errorCert = X509_STORE_CTX_get_current_cert(vfyCrtCtx);
